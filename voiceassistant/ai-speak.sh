...
if [ "$IS_MULTI_SPEAKER" = true ]; then
    echo "$TEXT" | "$PIPER_DIR/piper" --model "$VOICE" --speaker "$SPEAKER_ID" --output_file /tmp/speech.wav
else
    echo "$TEXT" | "$PIPER_DIR/piper" --model "$VOICE" --output_file /tmp/speech.wav
fi

aplay -q /tmp/speech.wav
rm /tmp/speech.wav
